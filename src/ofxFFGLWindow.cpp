...
#else
void ofxFFGLWindow::setup(const ofGLWindowSettings & settings){
#endif
...
void ofxFFGLWindow::setup(const ofGLFWWindowSettings & _settings){
	if(windowP){
	...
	if(settings.windowMode==OF_GAME_MODE){
		...
		if(count>settings.monitor){
			...
			ofLogError("ofxFFGLWindow") << "couldn't find any monitors";
			return;
		}
	}else{
		windowP = glfwCreateWindow(settings.width, settings.height, settings.title.c_str(), nullptr, sharedContext);
		if(!windowP){
			ofLogError("ofxFFGLWindow") << "couldn't create GLFW window";
			return;
		}
...
#ifdef TARGET_LINUX
//------------------------------------------------------------
void ofxFFGLWindow::setWindowIcon(const string & path){
    ofPixels iconPixels;
...
//------------------------------------------------------------
void ofxFFGLWindow::setWindowIcon(const ofPixels & iconPixels){
	iconSet = true;
...
//--------------------------------------------
ofCoreEvents & ofxFFGLWindow::events(){
	return coreEvents;
}

//--------------------------------------------
shared_ptr<ofBaseRenderer> & ofxFFGLWindow::renderer(){
	return currentRenderer;
}

//--------------------------------------------
void ofxFFGLWindow::update(){
	events().notifyUpdate();
...
//--------------------------------------------
bool ofxFFGLWindow::getWindowShouldClose(){
...
//------------------------------------------------------------
void ofxFFGLWindow::setWindowTitle(string title){
	glfwSetWindowTitle(windowP,title.c_str());
}

//------------------------------------------------------------
int ofxFFGLWindow::getPixelScreenCoordScale(){
    return pixelScreenCoordScale;
}

//------------------------------------------------------------
ofPoint ofxFFGLWindow::getWindowSize(){
...
//------------------------------------------------------------
ofPoint ofxFFGLWindow::getWindowPosition(){
    int x, y; 
...
//------------------------------------------------------------
int ofxFFGLWindow::getCurrentMonitor(){
...
//------------------------------------------------------------
ofPoint ofxFFGLWindow::getScreenSize(){
	int count;
	GLFWmonitor** monitors = glfwGetMonitors(&count);
	if(count>0){
...
//------------------------------------------------------------
GLFWwindow* ofxFFGLWindow::getGLFWWindow(){
    return windowP;
}

//------------------------------------------------------------
ofWindowMode ofxFFGLWindow::getWindowMode(){
	return windowMode;
}

//------------------------------------------------------------
void ofxFFGLWindow::setWindowPosition(int x, int y){
    glfwSetWindowPos(windowP,x/pixelScreenCoordScale,y/pixelScreenCoordScale);
}

//------------------------------------------------------------
void ofxFFGLWindow::setWindowShape(int w, int h){
	#ifdef TARGET_OSX
...
//------------------------------------------------------------
void ofxFFGLWindow::enableSetupScreen(){
	bEnableSetupScreen = true;
};

//------------------------------------------------------------
void ofxFFGLWindow::disableSetupScreen(){
	bEnableSetupScreen = false;
};

//------------------------------------------------------------
void ofxFFGLWindow::setFullscreen(bool fullscreen){
...
//------------------------------------------------------------
void ofxFFGLWindow::toggleFullscreen(){
...
//------------------------------------------------------------
void ofxFFGLWindow::setOrientation(ofOrientation orientation){
	this->orientation = orientation;
}

//------------------------------------------------------------
ofOrientation ofxFFGLWindow::getOrientation(){
	return orientation;
}
...
//------------------------------------------------------------
ofxFFGLWindow * ofxFFGLWindow::setCurrent(GLFWwindow* windowP){
	ofxFFGLWindow * instance = static_cast<ofxFFGLWindow *>(glfwGetWindowUserPointer(windowP));
...
//------------------------------------------------------------
void ofxFFGLWindow::mouse_cb(GLFWwindow* windowP_, int button, int state, int mods) {
	ofxFFGLWindow * instance = setCurrent(windowP_);
#ifdef TARGET_OSX
...
//------------------------------------------------------------
void ofxFFGLWindow::motion_cb(GLFWwindow* windowP_, double x, double y) {
	ofxFFGLWindow * instance = setCurrent(windowP_);
...
//------------------------------------------------------------
void ofxFFGLWindow::entry_cb(GLFWwindow *windowP_, int entered) {
	ofxFFGLWindow * instance = setCurrent(windowP_);
	if(entered){
...
//------------------------------------------------------------
void ofxFFGLWindow::scroll_cb(GLFWwindow* windowP_, double x, double y) {
	ofxFFGLWindow * instance = setCurrent(windowP_);
	rotateMouseXY(instance->orientation, instance->getWidth(), instance->getHeight(), x, y);
...
//------------------------------------------------------------
void ofxFFGLWindow::drop_cb(GLFWwindow* windowP_, int numFiles, const char** dropString) {
	ofxFFGLWindow * instance = setCurrent(windowP_);
	ofDragInfo drag;
...
//------------------------------------------------------------
void ofxFFGLWindow::error_cb(int errorCode, const char* errorDescription){
	ofLogError("ofxFFGLWindow") << errorCode << ": " << errorDescription;
}

//------------------------------------------------------------
void ofxFFGLWindow::keyboard_cb(GLFWwindow* windowP_, int keycode, int scancode, unsigned int codepoint, int action, int mods) {
	int key;
	switch (keycode) {
...
//------------------------------------------------------------
void ofxFFGLWindow::resize_cb(GLFWwindow* windowP_,int w, int h) {
	ofxFFGLWindow * instance = setCurrent(windowP_);
	instance->windowW = w;
	instance->windowH = h;
...
//--------------------------------------------
void ofxFFGLWindow::exit_cb(GLFWwindow* windowP_){
	ofxFFGLWindow * instance = setCurrent(windowP_);
	instance->events().notifyExit();
}

//------------------------------------------------------------
void ofxFFGLWindow::setVerticalSync(bool bVerticalSync){
	if(bVerticalSync){
		glfwSwapInterval( 1);
	}else{
		glfwSwapInterval(0);
...
//------------------------------------------------------------
void ofxFFGLWindow::setClipboardString(const string& text) {
    glfwSetClipboardString(ofxFFGLWindow::windowP, text.c_str());
}

//------------------------------------------------------------
string ofxFFGLWindow::getClipboardString() {
	const char* clipboard = glfwGetClipboardString(ofxFFGLWindow::windowP);
...
//------------------------------------------------------------
void ofxFFGLWindow::listVideoModes(){
	glfwInit();
	int numModes;
...
//------------------------------------------------------------
void ofxFFGLWindow::listMonitors(){
	glfwInit();
	int count;
...
//------------------------------------------------------------
bool ofxFFGLWindow::isWindowIconified(){
	return glfwGetWindowAttrib(windowP, GLFW_ICONIFIED);
}

//------------------------------------------------------------
bool ofxFFGLWindow::isWindowActive(){
//	return glfwGetWindowParam(GLFW_ACTIVE);
	return true;
}

//------------------------------------------------------------
bool ofxFFGLWindow::isWindowResizeable(){
	return !glfwGetWindowAttrib(windowP, GLFW_RESIZABLE);
}

//------------------------------------------------------------
void ofxFFGLWindow::iconify(bool bIconify){
	if(bIconify)
...
void ofxFFGLWindow::makeCurrent(){
	//glfwMakeContextCurrent(windowP);
}

#if defined(TARGET_LINUX) && !defined(TARGET_RASPBERRY_PI)
Display* ofxFFGLWindow::getX11Display(){
	return glfwGetX11Display();
}

Window ofxFFGLWindow::getX11Window(){
	return glfwGetX11Window(windowP);
}
#endif

#if defined(TARGET_LINUX) && !defined(TARGET_OPENGLES)
GLXContext ofxFFGLWindow::getGLXContext(){
	return glfwGetGLXContext(windowP);
}
#endif

#if defined(TARGET_LINUX) && defined(TARGET_OPENGLES)
EGLDisplay ofxFFGLWindow::getEGLDisplay(){
	return glfwGetEGLDisplay();
}

EGLContext ofxFFGLWindow::getEGLContext(){
	return glfwGetEGLContext(windowP);
}

EGLSurface ofxFFGLWindow::getEGLSurface(){
	return glfwGetEGLSurface(windowP);
}
#endif

#if defined(TARGET_OSX)
void * ofxFFGLWindow::getNSGLContext(){
	return glfwGetNSGLContext(windowP);
}

void * ofxFFGLWindow::getCocoaWindow(){
	return glfwGetCocoaWindow(windowP);
}
#endif

#if defined(TARGET_WIN32)
HGLRC ofxFFGLWindow::getWGLContext(){
	return glfwGetWGLContext(windowP);
}

HWND ofxFFGLWindow::getWin32Window(){
